...
const TECH_ROWS = [
  {
    ...
    items: [
      ...
      { icon: "grok", name: "Grok" },
      { icon: "hermes-agent", name: "Hermes Agent" },
    ],
  },
  {
    label: "AI MEDIA",
    marker: MARKER.pink,
    items: [
      { icon: "suno", name: "Suno", note: "music" },
      { icon: "dreamina", name: "Dreamina", note: "animation" },
    ],
  },
];
...
// so marks like Codex and Suno stay visible in dark mode.
  ...
  const rows = TECH_ROWS.map((row, r) => {
    ...
    const items = row.items.map((item, i) => {
      ...
      const note = item.note
        ? `\n<text class="note" x="${center}" y="${top + 80}" text-anchor="middle">${escape(item.note)}</text>`
        : "";
      return `${inlineIcon(item.icon, center - TECH.icon / 2, top, TECH.icon, t.ink)}
<text class="name" x="${center}" y="${top + 64}" text-anchor="middle">${escape(item.name)}</text>${note}`;
    });
...
.note{font:500 10.5px ${MONO};fill:${t.muted}}
</style>
${rows.join("\n")}
</svg>
`;
